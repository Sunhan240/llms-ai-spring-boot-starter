 ...
 */
package com.eastrobot.arch.llms.chat.openai.metadata.support;

import com.eastrobot.arch.llms.chat.metadata.RateLimit;
import com.eastrobot.arch.llms.chat.openai.metadata.OpenAiRateLimit;
...
import static com.eastrobot.arch.llms.chat.openai.metadata.support.OpenAiApiResponseHeaders.*;
...
public class OpenAiResponseHeaderExtractor {
	...
	enum DurationFormatter {
		...
		public Duration parse(String text) {
			...
			Assert.hasText(text, String.format("Text [%s] to parse as a Duration must not be null or empty", text));
			...
			static Unit parseUnit(String value) {
				...
				return Arrays.stream(values())
					...
					.orElseThrow(() -> new IllegalStateException(
							String.format("Value [%s] does not contain a valid time unit", value)));
			}

			private static String parse(String value, Predicate<Character> predicate) {
				Assert.hasText(value, String.format("Value [%s] must not be null or empty", value));
